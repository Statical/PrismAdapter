...
public async Task<NavObjectLicenseStatus> ExportSingleAsync(NavObjectReference navObjectRef, Stream outStream, CancellationToken cancellationToken)
    ...
    public async Task DesignObjectAsync(NavObjectReference navObjectRef)
    {
        ...
        try
        {
            ...
            var args = new StringBuilder()
                ...
                .Append(Param("servername", Context.DbServer)).Append(',')
                .Append(Param("database", Context.DbName)).Append(',');
            if (Context.IsNtAuthentication)
            {
            ...
            else
            {
                args
                    ...
                    .Append(Param("username", Context.DbUserId)).Append(',')
                    .Append(Param("password", Context.DbPassword)).Append(',');
            }
            ...
            process.StartInfo.Arguments = " /S /C \"" + Quote(FinsqlFullName) + " " + args.ToString() + "\"";
    ...
    protected virtual string FinSqlArguments(string filter, bool singleObject, string filePath, string tmpLogFile)
    {
        StringBuilder args = new StringBuilder()
            ...
            .Append(Param("servername", Context.DbServer)).Append(',')
            .Append(Param("database", Context.DbName)).Append(',');
        if (!string.IsNullOrWhiteSpace(Context.NavServerName) && !string.IsNullOrWhiteSpace(Context.NavServerInstance))
        {
            args
                .Append(Param("navservername", Context.NavServerName)).Append(',')
                .Append(Param("navserverinstance", Context.NavServerInstance)).Append(',')
                .Append(Param("navservermanagementport", Context.NavServerManagementPort.ToString())).Append(',');
        }
        if (Context.IsNtAuthentication)
        {
            args
    ...
    private string Param(string parameterName, string parameterValue) => " " + parameterName + "=" + Quote(parameterValue);
