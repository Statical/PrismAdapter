...
namespace Statical.NavAdapter
{
    ...
    public class NavVersionListFilter
    {
        ...
        public static void AddSqlParameters(SqlCommand command, ISet<NavVersionListFilter> versionExclusions)
        {
            for (int i = 0; i < versionExclusions.Count; i++)
            {
                SqlParameter param = new SqlParameter(SqlParameterName(i), SqlDbType.VarChar, 250);
                param.Value = versionExclusions.ElementAt(i).Filter.Replace('*', '%');
        ...
        private static string SqlParameterName(int i)
        {
            return "@versionFilter" + i.ToString();
        }
